...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, onApprove, onReject, onToggleApproval, onDuplicate, onDelete }) => {
    const imageUrl = (() => {
    ...
    return (
        <div className="group relative bg-white rounded-[20px] shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 w-full max-w-[320px] mx-auto overflow-hidden">
            ...
            <div className="relative w-full h-[180px] p-[8px]">
                    ...
                    <div className="absolute top-[16px] right-[16px] z-10">
                        <div className="flex bg-white/95 backdrop-blur-sm rounded-full p-1 shadow-lg">
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-200 ${property.approvalStatus === 'approved'
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-200 ${property.approvalStatus === 'not_approved'
                ...
                {/* Edit Button - Show on hover at top left */}
                {onToggleApproval && (
                    <div className="absolute top-[16px] left-[16px] flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                        <Link
                            to={`/landlord/properties/${property._id}/edit`}
                            onClick={(e) => e.stopPropagation()}
                            className="p-2 bg-white/95 backdrop-blur-sm rounded-full text-blue-600 hover:text-blue-800 hover:bg-blue-50 shadow-lg transition-all"
                            title="Edit Property"
                        >
                        ...
                        {onDuplicate && (
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    onDuplicate(property._id);
                                }}
                                className="p-2 bg-white/95 backdrop-blur-sm rounded-full text-green-600 hover:text-green-800 hover:bg-green-50 shadow-lg transition-all"
                        ...
                        {onDelete && (
                            <button
                                ...
                                className="p-2 bg-white/95 backdrop-blur-sm rounded-full text-red-600 hover:text-red-800 hover:bg-red-50 shadow-lg transition-all"
                ...
                {/* Legacy Landlord Actions - Show on hover (for backward compatibility) */}

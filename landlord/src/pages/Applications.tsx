...
export default function Applications() {
  ...
  const loadApplications = async () => {
    try {
      setLoading(true);
      const response = await landlordApi.getApplications(filters);
  ...
  const handleApproveApplication = async (applicationId: string) => {
    try {
      ...
      const response = await landlordApi.updateApplicationStatus(applicationId, 'approved');
  ...
  const handleRejectApplication = async (applicationId: string) => {
    ...
    try {
      ...
      const response = await landlordApi.updateApplicationStatus(applicationId, 'rejected', reason);
    ...
    try {
      await Promise.all(
        selectedApplications.map(id =>
          landlordApi.updateApplicationStatus(id, action)
        )
      );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <span className="landlord-badge bg-yellow-100 text-yellow-800">Pending</span>;
      case 'under_review':
        return <span className="landlord-badge bg-blue-100 text-blue-800">Under Review</span>;
      case 'approved':
        return <span className="landlord-badge-success">Approved</span>;
      case 'rejected':
        return <span className="landlord-badge-danger">Rejected</span>;
      default:
  ...
  const getPriorityBadge = (score: number) => {
    if (score >= 80) return <span className="landlord-badge bg-green-100 text-green-800">High Priority</span>;
    if (score >= 60) return <span className="landlord-badge bg-yellow-100 text-yellow-800">Medium Priority</span>;
    return <span className="landlord-badge bg-red-100 text-red-800">Low Priority</span>;
  };
    ...
    return (
      <div className="group relative bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border border-gray-100">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            to={`/landlord/applications/${application._id}`}
            ...
            <table className="landlord-table">
              <thead>
                <tr>
                  ...
                  <th className="landlord-table-header">Applicant</th>
                  ...
                  <th className="landlord-table-header">Move-in Date</th>
                  <th className="landlord-table-header">Applied</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {applications.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="landlord-table-cell text-center text-gray-500 py-8">

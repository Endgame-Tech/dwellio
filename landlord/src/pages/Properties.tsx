...
export default function Properties() {
  ...
  const loadProperties = async () => {
    try {
      setLoading(true);
      const response = await landlordApi.getProperties(filters);
  ...
  const handleApproveProperty = async (propertyId: string) => {
    try {
      ...
      const response = await landlordApi.approveProperty(propertyId);
  ...
  const handleRejectProperty = async (propertyId: string) => {
    ...
    try {
      ...
      const response = await landlordApi.rejectProperty(propertyId, reason);
  ...
  const handleToggleApproval = async (propertyId: string, newStatus: 'approved' | 'not_approved') => {
    try {
      ...
      if (newStatus === 'approved') {
        ...
        const response = await landlordApi.rejectProperty(propertyId, reason || 'Not approved by landlord');
        console.log('Reject property response:', response);
  ...
  const handleDuplicateProperty = async (propertyId: string) => {
    try {
      const confirmed = window.confirm('Do you want to duplicate this property?');
      if (!confirmed) return;

      console.log('Duplicating property:', propertyId);
      const response = await landlordApi.duplicateProperty(propertyId);
      console.log('Duplicate property response:', response);
      toast.success('Property duplicated successfully');
      loadProperties();
    } catch (error: any) {
      console.error('Failed to duplicate property:', error);
      const errorMessage = error.response?.data?.message || error.message || 'Failed to duplicate property';
      toast.error(errorMessage);
    }
  };
  ...
  const handleDeleteProperty = async (propertyId: string) => {
    try {
      ...
      const response = await landlordApi.deleteProperty(propertyId);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'available':
        return <span className="landlord-badge-success">Available</span>;
      case 'occupied':
        return <span className="landlord-badge-warning">Occupied</span>;
      case 'pending':
        return <span className="landlord-badge-info">Pending</span>;
      default:
        return <span className="landlord-badge-info">{status}</span>;
    }
  };
  ...
  const getTypeBadge = (type: string) => {
    ...
    return (
      <span className={`landlord-badge ${typeColors[type] || 'bg-gray-100 text-gray-800'}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link to="/landlord/properties/new" className="landlord-btn-primary flex items-center">
            ...
            <table className="landlord-table">
              <thead>
                <tr>
                  ...
                  <th className="landlord-table-header">Type</th>
                  <th className="landlord-table-header">Location</th>
                  <th className="landlord-table-header">Rent</th>
                  <th className="landlord-table-header">Landlord</th>

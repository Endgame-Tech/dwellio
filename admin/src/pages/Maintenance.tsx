...
export default function Maintenance() {
  ...
  const loadMaintenanceRequests = async () => {
    try {
      setLoading(true);
      const response = await adminApi.getMaintenanceRequests(filters);
  ...
  const handleUpdateStatus = async (requestId: string, status: string, notes?: string) => {
    try {
      ...
      const response = await adminApi.updateMaintenanceStatus(requestId, status, notes);
  ...
  const handleAssignTechnician = async (requestId: string) => {
    ...
    try {
      ...
      const response = await adminApi.assignMaintenance(requestId, technicianId);
    ...
    try {
      await Promise.all(
        selectedRequests.map(id => 
          adminApi.updateMaintenanceStatus(id, action)
        )
      );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'submitted':
        return <span className="admin-badge bg-gray-100 text-gray-800">Submitted</span>;
      case 'acknowledged':
        return <span className="admin-badge bg-blue-100 text-blue-800">Acknowledged</span>;
      case 'in_progress':
        return <span className="admin-badge bg-yellow-100 text-yellow-800">In Progress</span>;
      case 'completed':
        return <span className="admin-badge-success">Completed</span>;
      case 'cancelled':
        return <span className="admin-badge-danger">Cancelled</span>;
      default:
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'low':
        return <span className="admin-badge bg-green-100 text-green-800">Low</span>;
      case 'medium':
        return <span className="admin-badge bg-yellow-100 text-yellow-800">Medium</span>;
      case 'high':
        return <span className="admin-badge bg-orange-100 text-orange-800">High</span>;
      case 'urgent':
        return <span className="admin-badge bg-red-100 text-red-800">🚨 Urgent</span>;
      default:
        return <span className="admin-badge bg-gray-100 text-gray-800">{priority}</span>;
    }
  };
    ...
    return (
      <div className="group relative bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border border-gray-100">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            to={`/admin/maintenance/${request._id}`}
            ...
            <table className="admin-table">
              <thead>
                <tr>
                  ...
                  <th className="admin-table-header">Request</th>
                  <th className="admin-table-header">Tenant</th>
                  <th className="admin-table-header">Property</th>
                  <th className="admin-table-header">Category</th>
                  <th className="admin-table-header">Priority</th>
                  <th className="admin-table-header">Status</th>
                  <th className="admin-table-header">Cost</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {maintenanceRequests.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="admin-table-cell text-center text-gray-500 py-8">

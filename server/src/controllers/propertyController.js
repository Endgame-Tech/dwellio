...
const getPropertyById = async (req, res) => {
  try {
    ...
    if (moveOutIntent.status === 'pending') {
      return res.status(403).json({ 
        message: 'Complete your move-out request payment to view property details',
...
const checkMoveOutEligibility = async (req, res) => {
  try {
    ...
    const moveOutIntent = user.tenantProfile?.moveOutIntent;
    const hasActiveIntent = moveOutIntent && moveOutIntent.facilitationRequested;
...
const getPropertyAreas = async (req, res) => {
  try {
    ...
    const propertyTypes = await Property.distinct('type', { status: 'available', isActive: true });
    
    res.json({
      success: true,
      data: {
        availableAreas: areas[0] || { cities: [], lgas: [], minPrice: 0, maxPrice: 0 },
        propertyTypes,
        totalAvailableProperties: await Property.countDocuments({ status: 'available', isActive: true })
      }
    });
  } catch (error) {

...
userSchema.pre('save', async function (next) {
  ...
  if (this.isModified('tenantProfile') || this.isModified('verificationStatus') || this.isModified('creditHistory')) {
...
userSchema.methods.calculateTrustScore = function() {
  ...
  if (this.verificationStatus === 'verified') {
    ...
    const verifiedDocs = this.tenantProfile?.documents?.filter(doc => doc.verified).length || 0;
  ...
  if (this.tenantProfile?.currentResidence?.address && 
  ...
  if (this.tenantProfile?.occupation && this.tenantProfile?.employerName && this.tenantProfile?.monthlyIncome) {
    score += 10;
  }

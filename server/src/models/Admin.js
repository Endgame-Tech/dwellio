...
const adminPermissionSchema = new mongoose.Schema({
  resource: {
    ...
    required: true,
    enum: [
      'users', 'properties', 'applications', 'payments', 'analytics', 
      'settings', 'admin_management', 'system_logs', 'reports'
    ]
  },
  actions: [{
    type: String,
...
const adminSchema = new mongoose.Schema({
  ...
  profile: {
    ...
    department: { 
      type: String, 
      enum: ['Operations', 'Customer Support', 'Analytics', 'Security', 'Management'] 
    },
    phoneNumber: {
      type: String,
      trim: true,
...
// Log admin activity
adminSchema.methods.logActivity = async function(action, resource, resourceId = null, details = null, req = null) {
  const activityEntry = {
    action,
    resource,
